  ...
  return (
    <header
      className={`sticky top-0 z-50 transition-all duration-300 ${
        sticky
          ? 'bg-brand-navyDark/95 backdrop-blur-md shadow-lg shadow-brand-navyDark/40'
          : 'bg-gradient-to-r from-brand-deep to-brand-navy'
      }`}
    >
      <nav className="mx-auto max-w-7xl px-4 lg:px-8 h-20 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="hidden lg:inline-flex items-center gap-2 bg-brand-gold text-brand-navyDark hover:bg-brand-navy hover:text-white text-sm font-bold rounded-full px-5 py-2.5 shadow-glow hover:scale-105 transition-all"
          >
            🧭 {SITE.cta.label}
          </button>
          <button
      ...
      {/* mobile panel */}
      {open && (
        <div className="lg:hidden anim-fade absolute left-0 right-0 top-full bg-brand-navyDark/95 backdrop-blur-md border-b border-white/10 shadow-2xl">
          <div className="px-4 py-3">
            {SITE.navLinks.map((l) => (
              <button
                ...
                className={`w-full text-left px-4 py-3 rounded-xl font-medium transition-colors ${
                  active === l.id ? 'bg-brand-gold/15 text-brand-gold' : 'text-white/85 hover:text-brand-gold'
                }`}
              >
                {l.label}
              </button>
            ...
            <button
              onClick={() => go(SITE.cta.target)}
              className="mt-2 w-full text-center bg-brand-gold text-brand-navyDark font-bold rounded-full px-5 py-3"
            >
              🧭 {SITE.cta.label}
            </button>
          </div>
